...
module.exports.handleReply = async function({ api, event, handleReply }) {
    ...
    switch(handleReply.step) {
        case 1: {
            if(isNaN(event.body)) return api.sendMessage('يجب عليك إدخال رقم', event.threadID, event.messageID)   
            api.unsendMessage(handleReply.messageID);
            return api.sendMessage(`اخترت حرفًا بمعرف ${event.body}, يرجى الرد على هذه الرسالة لإدخال الاسم الرئيسي`, event.threadID, (err, info) => {
        ...
        case 2: {
            api.unsendMessage(handleReply.messageID);
            return api.sendMessage(`اخترت الاسم الرئيسي${event.body}, يرجى الرد على هذه الرسالة لإدخال الاسم`, event.threadID, (err, info) => {
                global.client.handleReply.push({
                    step: 3,
                    name: this.config.name,
                    author: event.senderID,
...
module.exports.run = async function({ api, event }) {
    ...
    return api.sendMessage("رد على هذه الرسالة لاختيار الرقم", event.threadID, (err, info) => {
        global.client.handleReply.push({
            step: 1,
            name: this.config.name,
            author: event.senderID,
