...
module.exports.run = async ({
    ...
    if ((this.config.credits) != "amiroux") { return api.sendMessage(`تم تغيير الاعتمادات المكتشفة`, event.threadID, event.messageID)}
    const {
        configPath
    } = global.client;
    const {
    ...
    if (modDev == true) {
        api.sendMessage(`وضع المطور: ${modDev}\nيرجى ضبطه على خطأ للاستخدام!!!`, event.threadID)
    } else
