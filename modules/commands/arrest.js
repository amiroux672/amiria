module.exports.config = {
    ...
    hasPermssion: 0,
    credits: "",
    description: "القبض على صديق ذكرته",
...
module.exports.run = async function ({ event, api, args }) {
    ...
    if (!mention) return api.sendMessage("يرجى ذكر شخص واحد", threadID, messageID);
    else {
        var one = senderID, two = mention;
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "تم قبض على المجرم" + tag + '\n أتمنى لك السعادة فسجن',
            mentions: [{
          tag: tag,
          id: mention
        }],
