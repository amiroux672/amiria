...
var min = Math.floor(Math.random() * 2);
  ...
  var money = data.money
      if(money < 0) api.sendMessage("تحتاج إلى 100 دولار لرؤية الصورة ?",event.threadID,event.messageID)
          else {
   Currencies.setData(event.senderID, options = {money: money - 0})
   var callback = () => api.sendMessage({body:`-100 تم خصم ظولار😬`,attachment: fs.createReadStream(__dirname + "/cache/1.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/1.jpg"), event.messageID); 
