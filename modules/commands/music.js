...
module.exports.handleReply = async function({ api, event, handleReply }) {
	...
	api.sendMessage(`معالجة الصوت !\n◆━━━━━━━━━━━━━◆\n${body}\n◆━━━━━━━━━━━━━◆\n انتظر من فضلك !`, event.threadID, (err, info) =>
	...
	try {
		...
		ytdl(handleReply.link[event.body - 1])
			...
			.on("close", () => {
				if (statSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.m4a`).size > 26214400) return api.sendMessage('تعذر إرسال الملف لأن حجمه أكبر من 25 ميغابايت.', event.threadID, () => unlinkSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.m4a`), event.messageID);
	...
	if (urlValid) {
		try {
			...
			ytdl(args[0])
				...
				.on("close", () => {
					if (statSync(__dirname + `/cache/${id}.m4a`).size > 26214400) return api.sendMessage('تعذر إرسال الملف لأن حجمه أكبر من 25 ميغابايت.', event.threadID, () => unlinkSync(__dirname + `/cache/${id}.m4a`), event.messageID);
		...
		try {
			...
			body = `عنوان: ${songInfo.title} | ${(timePlay - (timePlay %= 60)) / 60 + (9 < timePlay ? ':' : ':0') + timePlay}]`;
		}
		catch (error) {
			if (error.statusCode == "404") return api.sendMessage("لا يمكن العثور على أغنيتك عبر الرابط أعلاه ;w;", event.threadID, event.messageID);
api.sendMessage("تعذرت معالجة الطلب بسبب حدوث خطأ: " + error.message, event.threadID, event.messageID);
		}
		try {
		...
		try {
			...
			return api.sendMessage(`هناك ${link.length} النتائج المطابقة لمصطلح البحث الخاص بك: \n${msg}\nيرجى الرد (التعليقات) واختيار إحدى عمليات البحث المذكورة أعلاه\nالحد الأقصى لوقت الأغنية 10 ساعات!`, event.threadID,(error, info) => global.client.handleReply.push({ name: this.config.name, messageID: info.messageID, author: event.senderID, link }), event.messageID);
		}
		catch (error) {
			api.sendMessage(" تعذرت معالجة الطلب بسبب خطأ:" + error.message, event.threadID, event.messageID);
		}
	}
}