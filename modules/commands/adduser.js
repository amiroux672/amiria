...
module.exports.run = async function ({ api, event, args }) {
	...
	if (!args[0]) return out("الرجاء إدخال معرف/رابط واحد يحتاج المستخدم إلى إضافته.");
	if (!isNaN(args[0])) return adduser(args[0], undefined);
	else {
		try {
			var [id, name, fail] = await getUID(args[0], api);
			if (fail == true && id != null) return out(id);
			else if (fail == true && id == null) return out("لم يتم العثور على هوية المستخدم.")
			else {
	...
	async function adduser(id, name) {
		id = parseInt(id);
		if (participantIDs.includes(id)) return out(`${name ? name : "عضو"} موجودون بالفعل في المجموعة.`);
		else {
			...
			try {
				...
				return out(`لا يمكن الإضافة ${name ? name : "user"} الى المجموعة.`);
			}
			if (approvalMode === true && !admins.includes(botID)) return out(`يضيف ${name ? name : "member"}إلى القائمة المعتمدة!`);
			else return out(`تمت الإضافة ${name ? name : "member"} في المجموعة !`)
		}
	}
    }