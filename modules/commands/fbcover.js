...
hasPermssion: 0,
credits: "D",
description: "إنشاء صورة غلاف الفيسبوك",
...
if ((this.config.credits) != "amirX") { return api.sendMessage(`[ WARNING ] - الكشف عن وحدات الاعتمادات ${this.config.name} تم تغييره إلى ${this.config.credits} بواسطة أدمينبوت ${global.config.BOTNAME}  توقف على الفور!!!!`, event.threadID, event.messageID)}
    const request = require('request');
  const fs = require("fs-extra")
  const axios = require("axios")
  const { threadID, messageID, senderID, body } = event;
  if(!args[0]) return api.sendMessage('الرجاء إدخال الاسم الرئيسي!!!', threadID, messageID)
  else return api.sendMessage(`لقد اخترت الاسم الرئيسي: ${args.join(" ").toUpperCase()}\n\n(قم بالرد على هذه الرسالة واختر اسمك الثانوي)`,event.threadID, (err, info) => {
...
module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
  ...
  switch (handleReply.type) {
    case "tenphu": {
      ...
      return api.sendMessage(`لقد قمت باختيار اسم فرعي ${event.body.toUpperCase()}\n\n(قم بالرد على هذه الرسالة أدخل رقم هاتفك)`,threadID, function (err, info) {
        return global.client.handleReply.push({
          type: "sdt",
          name: `fbcover`,
          author: senderID,
    ...
    case "sdt": {
      api.unsendMessage(handleReply.messageID);
      return api.sendMessage(` لقد حددت المعاملة الخاصة والتفضيلية كـ: ${event.body.toUpperCase()}\n\n(قم بالرد على هذه الرسالة لإدخال بريدك الإلكتروني)`,threadID, function (err, info) {
        return global.client.handleReply.push({
          type: "email",
          name: `fbcover`,
          author: senderID,
    ...
    case "email": {
      api.unsendMessage(handleReply.messageID);
      return api.sendMessage(` لقد قمت بتحديد البريد الإلكتروني ك : ${event.body.toUpperCase()}\n\n(قم بالرد على هذه الرسالة لإدخال عنوانك)`,threadID, function (err, info) {
        return global.client.handleReply.push({
          type: "color",
          name: `fbcover`,
          author: senderID,
    ...
    case "color": {
      api.unsendMessage(handleReply.messageID);
      return api.sendMessage(` لقد اخترت العنوان كما : ${event.body.toUpperCase()}\nقم بالرد على هذه الرسالة لإدخال لون الخلفية (أدخل لا كاللون الافتراضي)`,threadID, function (err, info) {
        return global.client.handleReply.push({
          type: "create",
          name: `fbcover`,
          author: senderID,
    ...
    case "create": {
      ...
      api.unsendMessage(handleReply.messageID);
      api.sendMessage(`鈴� تهيئة صانع الصور...`,threadID, (err, info) => {
      setTimeout(() => {
